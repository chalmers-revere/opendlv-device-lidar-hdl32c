...
// Calibration data for HDL32c.
const char *HDL32c_XML = R"(
...
HDL32cDecoder::HDL32cDecoder(int32_t intensity) noexcept
    : m_intensityBitsMSB(intensity) {
    setupCalibration();
    index32sensorIDs();
}

void HDL32cDecoder::setupCalibration() noexcept {
    ...
    const std::string CALIBRATION(HDL32c_XML);
...
void HDL32cDecoder::index32sensorIDs() noexcept {
...
std::vector<opendlv::proxy::PointCloudReading> HDL32cDecoder::decode(const std::string &data) noexcept {
            ...
            if (m_pointIndexCPC < MAX_POINT_SIZE) {
                ...
                for (uint8_t counter{0}; counter < 32; counter++) {
                    ...
                    if (m_pointIndexCPC >= MAX_POINT_SIZE) {
                        ...
                        std::cerr << "[lidar-hdl32c] More than 70,000 points?" << std::endl; 
                        break;
                    }
                }
            }
