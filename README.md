## OpenDLV Microservice to interface with VelodyneLidar HDL32c units

This repository provides source code to interface with a VelodyneLidar HDL32c
unit for the OpenDLV software ecosystem.

[![Build Status](https://travis-ci.org/chalmers-revere/opendlv-device-lidar-hdl32c.svg?branch=master)](https://travis-ci.org/chalmers-revere/opendlv-device-lidar-hdl32c) [![License: GPLv3](https://img.shields.io/badge/license-GPL--3-blue.svg
...
* [x86_64](https://hub.docker.com/r/chalmersrevere/opendlv-device-lidar-hdl32c-amd64/tags/)
* [armhf](https://hub.docker.com/r/chalmersrevere/opendlv-device-lidar-hdl32c-armhf/tags/)
* [aarch64](https://hub.docker.com/r/chalmersrevere/opendlv-device-lidar-hdl32c-aarch64/tags/)

To run this microservice using our pre-built Docker multi-arch images to connect
to a VelodyneLidar HDL32c unit broadcasting data to `0.0.0.0:2368` and to publish
...
```
docker run --init --rm --net=host chalmersrevere/opendlv-device-lidar-hdl32c-multi:v0.0.1 opendlv-device-lidar-hdl32c --hdl32c_ip=0.0.0.0 --hdl32c_port=2368 --cid=111 --verbose
```
...
`opendlv-device-lidar-hdl32c` receives the data from HDL-32C (Velodyne LiDAR with 32 layers)
...
by HDL-32C per scan is more than double the size of VLP-16. Assuming 10Hz rotation rate, HDL-32C
is able to fire up to 70,000 points, which cannot be all stored in one single UDP packet. Therefore,
`opendlv-device-lidar-hdl32c` stores the data of a complete scan in three separate `PointCloudReading`
...
`PointCloudReading` message with 11 layers implies the second CPC from a HDL-32C unit.
